...
{
    public partial class frmSelectCustomer :frmSelectForm
    {
        string _idColName = "colCompanyGuID";
        string _chkColName = "colSelect";
        public CRM_CompanyLogic companyLogic=null;
        public frmSelectCustomer(bool isMutiSelect,string idColName)
        {
            InitializeComponent();
        ...
        void BindDgvCompany(int pageIndex)
        {
            ...
            if (!string.IsNullOrEmpty(txtCompanyCode.Text)) param.CompanyCode = txtCompanyCode.Text.Trim();
                ...
                param.PageSize = 100;
                PageList<CRM_CompanyResult> lstRst = companyLogic.GetCustPageList(param);
                dgvEmp.DataSource = lstRst.ResultList;
